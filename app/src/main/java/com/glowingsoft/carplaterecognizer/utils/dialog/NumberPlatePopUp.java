package com.glowingsoft.carplaterecognizer.utils.dialog;
...
public class NumberPlatePopUp extends AppCompatDialogFragment {
    private static final int NUMBER_PLATE_POPUP_REQUEST_CODE = 106;
...
//        String formattedNumberPlate = firstThreeCharacters + " " + remainingCharacters;
        ...
        builder.setView(view)
                .setTitle(R.string.numberplate_pop_title)
                .setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        Intent intent = new Intent();
                        listener.onActivityResult(NUMBER_PLATE_POPUP_REQUEST_CODE, Activity.RESULT_CANCELED, intent);
                    }
                })
                .setPositiveButton("Add", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if (TextUtils.isEmpty(vehicleNumberStr)) {
                            vehicleNumber.setError("Please fill in your number plate");
                        } else {
                            Intent intent = new Intent();
                            intent.putExtra("vehicleNumber",vehicleNumberStr);
                            intent.putExtra("wheelerType",wheelerType);
                            listener.onActivityResult(NUMBER_PLATE_POPUP_REQUEST_CODE, Activity.RESULT_OK, intent);
                        }
